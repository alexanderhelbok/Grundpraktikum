...
for i in range(3):
    ...
    if i == 2:
        ...
        ax1.hlines(fmean, df["t"][3*rate], df["t"][int(11.5*rate)], label="Fit", color="red", linewidth=0.75)
        ax1.set_ylabel("F (N)")
        ax1.legend(borderaxespad=1, markerscale=4)
        ax2.scatter(df["t"], df["a"], color="black", label="Data", s=0.5)
        ax2.hlines(amean, df["t"][3*rate], df["t"][int(11.5*rate)], label="Fit", color="red", linewidth=0.75)
        ax2.set_ylabel(r"a (m/s$^2$)")
        ax2.set_ylim(-11, -9)
        ax2.legend(borderaxespad=1, markerscale=4)
        ax3.scatter(df["t"], df["F"]/df["a"], label="Data", s=0.5, color="black")
        ax3.hlines(m[i].n, df["t"][3*rate], df["t"][int(11.5*rate)], label="Fit", color="red", linewidth=0.75)
