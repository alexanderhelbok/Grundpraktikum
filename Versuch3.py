...
for i in range(3):
    ...
    amean = df["a"][4*rate:12*rate].mean()
    astd = df["a"][4*rate:12*rate].std()
    fmean = df["F"][4*rate:12*rate].mean()
    fstd = df["F"][4*rate:12*rate].std()

    F = unc.ufloat(fmean - df["F"][0], np.sqrt(2)*fstd)
    a = unc.ufloat(amean, np.sqrt(2)*astd)
    print(f"F: {F:.1uS}")
    print(f"a: {a:.1uS}")
    ...
    if i == 2:
        ...
        ax1.hlines(fmean, df["t"][3*rate], df["t"][12*rate], label="Fit", color="red", linewidth=0.75)
        ax1.set_ylabel("F [N]")
        ax1.legend(borderaxespad=1, markerscale=4)
        ax2.scatter(df["t"], df["a"], color="black", label="Data", s=0.5)
        ax2.hlines(amean, df["t"][3*rate], df["t"][12*rate], label="Fit", color="red", linewidth=0.75)
        ax2.set_ylabel(r"a [m/s$^2$]")
        ax2.set_ylim(-11, -9)
        ax2.legend(borderaxespad=1, markerscale=4)
        ax3.scatter(df["t"], df["F"]/df["a"], label="Data", s=0.5, color="black")
        ax3.hlines(m[i].n, df["t"][3*rate], df["t"][12*rate], label="Fit", color="red", linewidth=0.75)
...
for i in range(3):
    ...
    # w[i] = unc.ufloat(np.round(fit[1], 3), 0.1)
...
k = w**2*m
for i in range(3):
    print(f"k{i+1} = {k[i]:.1uS} m2{i+1} = {m[i]:.2uS}")
